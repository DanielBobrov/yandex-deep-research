# Yandex Deep Research Install

This file is for coding agents. If the Yandex Deep Research repository is not already cloned and open, clone `https://github.com/yandex/yandex-deep-research.git` first, then continue from the repository root.

## Goal

Bootstrap a Yandex Deep Research local development workspace on the user's machine with the least risky path available.
...
- The Yandex Deep Research repository is cloned and the current working directory is the repo root.
- `config.yaml` exists.
...
- The user receives the exact next command to launch Yandex Deep Research.
...
- If the current directory is not the Yandex Deep Research repository root, clone `https://github.com/yandex/yandex-deep-research.git` if needed, then change into the repository root.
- Confirm the current directory is the Yandex Deep Research repository root by checking that `Makefile`, `backend/`, `frontend/`, and `config.example.yaml` exist.
...
- If `config.yaml` references variables such as `$YANDEX_API_KEY`, tell the user which variable names still need real values, but do not verify them by opening secret-bearing files.
...
5. Exact next command to start Yandex Deep Research
