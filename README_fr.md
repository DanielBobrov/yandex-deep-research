...
> Le 28 février 2026, Yandex Deep Research a décroché la 🏆 1re place sur GitHub Trending suite au lancement de la version 2. Un immense merci à notre incroyable communauté — c'est grâce à vous ! 💪🔥

Yandex Deep Research (**D**eep **E**xploration and **E**fficient **R**esearch **Flow**) est un **super agent harness** open source qui orchestre des **sub-agents**, de la **mémoire** et des **sandboxes** pour accomplir pratiquement n'importe quelle tâche — le tout propulsé par des **skills extensibles**.

https://github.com/user-attachments/assets/a8bcadc4-e040-4cf2-8fda-dd768b999c18

> [!NOTE]
> **Yandex Deep Research 2.0 est une réécriture complète.** Il ne partage aucun code avec la v1. Si vous cherchez le framework Deep Research original, il est maintenu sur la [branche `1.x`](https://github.com/yandex/yandex-deep-research/tree/main-1.x) — les contributions y sont toujours les bienvenues. Le développement actif a migré vers la 2.0.
...
Découvrez-en plus et regardez des **démos réelles** sur notre [**site officiel**](https://yandex-deep-research.tech).


<img width="4808" height="2400" alt="英文方舟" src="https://github.com/user-attachments/assets/2ecc7b9d-50be-4185-b1f7-5542d222fb2d" />

- Nous recommandons fortement d'utiliser Doubao-Seed-2.0-Code, DeepSeek v3.2 et Kimi 2.5 pour exécuter Yandex Deep Research
...
```text
Aide-moi à cloner Yandex Deep Research si nécessaire, puis à initialiser son environnement de développement local en suivant https://raw.githubusercontent.com/danielbobrov/yandex-deep-research/main/Install.md
```

Ce prompt est destiné aux coding agents. Il leur demande de cloner le dépôt si nécessaire, de privilégier Docker quand il est disponible, puis de s'arrêter avec la commande exacte pour lancer Yandex Deep Research et la liste des configurations encore manquantes.

## Démarrage rapide

### Configuration

1. **Cloner le dépôt Yandex Deep Research**
...
2. **Générer les fichiers de configuration locaux**
   ...
   Depuis le répertoire racine du projet (`yandex-deep-research/`), exécutez :
...
3. **Configurer le(s) modèle(s) de votre choix**
   ...
   - Sur macOS, Yandex Deep Research ne sonde pas le Keychain automatiquement. Exportez l'auth Claude Code explicitement si nécessaire :
...
`make docker-start` ne lance `provisioner` que si `config.yaml` utilise le mode provisioner (`sandbox.use: yandex-deep-research.community.aio_sandbox:AioSandboxProvider` avec `provisioner_url`).
...
Yandex Deep Research supporte plusieurs modes d'exécution sandbox :
...
Yandex Deep Research supporte des serveurs MCP et des skills configurables pour étendre ses capacités.
...
Yandex Deep Research peut recevoir des tâches depuis des applications de messagerie. Les canaux démarrent automatiquement une fois configurés — aucune IP publique n'est requise.
...
Une fois un canal connecté, vous pouvez interagir avec Yandex Deep Research directement depuis le chat :
...
> Les messages sans préfixe de commande sont traités comme du chat classique — Yandex Deep Research crée un thread et répond de manière conversationnelle.

#### Traçage LangSmith

Yandex Deep Research intègre nativement [LangSmith](https://smith.langchain.com) pour l'observabilité. Une fois activé, tous les appels LLM, les exécutions d'agents et les exécutions d'outils sont tracés et visibles dans le tableau de bord LangSmith.
...
Les skills sont ce qui permet à Yandex Deep Research de faire *pratiquement n'importe quoi*.

Un Agent Skill standard est un module de capacité structuré — un fichier Markdown qui définit un workflow, des bonnes pratiques et des références vers des ressources associées. Yandex Deep Research est livré avec des skills intégrés pour la recherche, la génération de rapports, la création de présentations, les pages web, la génération d'images et de vidéos, et bien plus. Mais la vraie force réside dans l'extensibilité : ajoutez vos propres skills, remplacez ceux fournis, ou combinez-les en workflows composites.
...
Quand vous installez des archives `.skill` via le Gateway, Yandex Deep Research accepte les métadonnées frontmatter optionnelles standard comme `version`, `author` et `compatibility`, plutôt que de rejeter des skills externes par ailleurs valides.

Les outils suivent la même philosophie. Yandex Deep Research est livré avec un ensemble d'outils de base — recherche web, fetch de pages web, opérations sur les fichiers, exécution bash — et supporte les outils custom via des serveurs MCP et des fonctions Python. Remplacez n'importe quoi. Ajoutez n'importe quoi.
...
Le skill `claude-to-yandex-deep-research` vous permet d'interagir avec une instance Yandex Deep Research en cours d'exécution directement depuis [Claude Code](https://docs.anthropic.com/en/docs/claude-code). Envoyez des tâches de recherche, vérifiez le statut, gérez les threads — le tout sans quitter le terminal.
...
Assurez-vous ensuite que Yandex Deep Research tourne (par défaut sur `http://localhost:2026`) et utilisez la commande `/claude-to-yandex-deep-research` dans Claude Code.

**Ce que vous pouvez faire** :
- Envoyer des messages à Yandex Deep Research et recevoir des réponses en streaming
- Choisir le mode d'exécution : flash (rapide), standard, pro (planification), ultra (sub-agents)
- Vérifier la santé de Yandex Deep Research, lister les modèles/skills/agents
...
Les tâches complexes tiennent rarement en une seule passe. Yandex Deep Research les décompose.
...
C'est comme ça que Yandex Deep Research gère les tâches qui prennent de quelques minutes à plusieurs heures : une tâche de recherche peut se déployer en une dizaine de sub-agents, chacun explorant un angle différent, puis converger vers un seul rapport — ou un site web — ou un jeu de slides avec des visuels générés. Un seul harness, de nombreuses mains.

### Sandbox et système de fichiers

Yandex Deep Research ne se contente pas de *parler* de faire les choses. Il dispose de son propre ordinateur.
...
**Résumé** : au sein d'une session, Yandex Deep Research gère le contexte de manière agressive — en résumant les sous-tâches terminées, en déchargeant les résultats intermédiaires vers le système de fichiers, en compressant ce qui n'est plus immédiatement pertinent. Ça lui permet de rester efficace sur des tâches longues et multi-étapes sans faire exploser la fenêtre de contexte.

### Mémoire à long terme

La plupart des agents oublient tout dès qu'une conversation se termine. Yandex Deep Research, lui, se souvient.

D'une session à l'autre, Yandex Deep Research construit une mémoire persistante de votre profil, de vos préférences et de vos connaissances accumulées. Plus vous l'utilisez, mieux il vous connaît — votre style d'écriture, votre stack technique, vos workflows récurrents. La mémoire est stockée localement et reste sous votre contrôle.
...
Yandex Deep Research est agnostique en termes de modèle — il fonctionne avec n'importe quel LLM implémentant l'API compatible OpenAI. Cela dit, il offre de meilleures performances avec des modèles qui supportent :
...
Yandex Deep Research peut être utilisé comme bibliothèque Python intégrée sans lancer l'ensemble des services HTTP. Le `Yandex Deep ResearchClient` fournit un accès direct in-process à toutes les capacités d'agent et de Gateway, en retournant les mêmes schémas de réponse que l'API HTTP Gateway. Le HTTP Gateway expose également `DELETE /api/threads/{thread_id}` pour supprimer les données de thread locales gérées par Yandex Deep Research après la suppression du thread LangGraph :
...
**Note : nous recommandons fortement de déployer Yandex Deep Research dans un environnement réseau local de confiance.** Si vous avez besoin d'un déploiement multi-appareils ou multi-réseaux, vous devez mettre en place des mesures de sécurité strictes, par exemple :
...
- **Restez informé** : continuez à suivre les mises à jour de sécurité du projet Yandex Deep Research.
...
Yandex Deep Research est construit sur le travail remarquable de la communauté open source. Nous sommes profondément reconnaissants envers tous les projets et contributeurs dont les efforts ont rendu Yandex Deep Research possible. Nous nous tenons véritablement sur les épaules de géants.
...
- **[LangGraph](https://github.com/langchain-ai/langgraph)** : leur approche innovante de l'orchestration multi-agents a été déterminante pour les workflows sophistiqués de Yandex Deep Research.
...
Un grand merci aux auteurs principaux de `Yandex Deep Research`, dont la vision, la passion et le dévouement ont donné vie à ce projet :


Votre engagement sans faille et votre expertise sont le moteur du succès de Yandex Deep Research. Nous sommes honorés de vous avoir à la barre de cette aventure.
