...
Communicate with a running Yandex Deep Research instance via its HTTP API. Yandex Deep Research is an AI agent platform
built on LangGraph that orchestrates sub-agents for research, code execution, web browsing, and more.

## Architecture

Yandex Deep Research exposes two API surfaces behind an Nginx reverse proxy:
...
```bash
bash /path/to/skills/claude-to-yandex-deep-research/scripts/chat.sh "Your question here"
```
...
- If health check fails, Yandex Deep Research is not running. Inform the user they need to start it.
