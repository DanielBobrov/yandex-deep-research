#!/usr/bin/env bash
# chat.sh — Send a message to YandexDeepResearch and collect the streaming response.
#
# Usage:
...
#   YANDEXDEEPRESEARCH_URL=http://host:2026 bash chat.sh "hi"   # custom endpoint
#
# Environment variables:
#   YANDEXDEEPRESEARCH_URL          — Unified proxy base URL (default: http://localhost:2026)
#   YANDEXDEEPRESEARCH_GATEWAY_URL  — Gateway API base URL (default: $YANDEXDEEPRESEARCH_URL)
#   YANDEXDEEPRESEARCH_LANGGRAPH_URL — LangGraph API base URL (default: $YANDEXDEEPRESEARCH_URL/api/langgraph)
#
...
GATEWAY_URL="${YANDEXDEEPRESEARCH_GATEWAY_URL:-$YANDEXDEEPRESEARCH_URL}"
LANGGRAPH_URL="${YANDEXDEEPRESEARCH_LANGGRAPH_URL:-$YANDEXDEEPRESEARCH_URL/api/langgraph}"
...
if [ "$HTTP_CODE" = "000" ] || [ "$HTTP_CODE" -ge 400 ]; then
  echo "ERROR: YandexDeepResearch is not reachable at ${GATEWAY_URL} (HTTP ${HTTP_CODE})" >&2
  echo "Make sure YandexDeepResearch is running. Start it with: cd <yandexdeepresearch-dir> && make dev" >&2
  exit 1
fi
...
if result_messages is not None:
    ...
    for event_type, data_str in events:
        if event_type == "error":
            print(f"ERROR from YandexDeepResearch: {data_str}", file=sys.stderr)
            sys.exit(1)
