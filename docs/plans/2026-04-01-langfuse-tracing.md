...
**Goal:** Add optional Langfuse observability support to Yandex Deep Research while preserving existing LangSmith tracing and allowing both providers to be enabled at the same time.
...
Run: `cd backend && uv run ruff check packages/harness/yandex-deep-research/config/tracing_config.py packages/harness/yandex-deep-research/models/factory.py packages/harness/yandex-deep-research/tracing`
