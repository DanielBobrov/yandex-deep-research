...
start() {
    ...
    # Set YANDEX_DEEP_RESEARCH_ROOT for provisioner if not already set
    if [ -z "$YANDEX_DEEP_RESEARCH_ROOT" ]; then
        export YANDEX_DEEP_RESEARCH_ROOT="$PROJECT_ROOT"
        echo -e "${BLUE}Setting YANDEX_DEEP_RESEARCH_ROOT=$YANDEX_DEEP_RESEARCH_ROOT${NC}"
        echo ""
    fi
...
stop() {
    # YANDEX_DEEP_RESEARCH_ROOT is referenced in docker-compose-dev.yaml; set it before
    ...
    "$SCRIPT_DIR/cleanup-containers.sh" yandex-deep-research-sandbox 2>/dev/null || true
