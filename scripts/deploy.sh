...
COMPOSE_CMD=(docker compose -p yandex-deep-research -f "$DOCKER_DIR/docker-compose.yaml")
...
# ── YANDEX_DEEP_RESEARCH_HOME ────────────────────────────────────────────────────────────

if [ -z "$YANDEX_DEEP_RESEARCH_HOME" ]; then
    export YANDEX_DEEP_RESEARCH_HOME="$REPO_ROOT/backend/.yandex-deep-research"
fi
echo -e "${BLUE}YANDEX_DEEP_RESEARCH_HOME=$YANDEX_DEEP_RESEARCH_HOME${NC}"
mkdir -p "$YANDEX_DEEP_RESEARCH_HOME"

# ── YANDEX_DEEP_RESEARCH_REPO_ROOT (for skills host path in DooD) ───────────────────────

export YANDEX_DEEP_RESEARCH_REPO_ROOT="$REPO_ROOT"

# ── config.yaml ───────────────────────────────────────────────────────────────

if [ -z "$YANDEX_DEEP_RESEARCH_CONFIG_PATH" ]; then
    export YANDEX_DEEP_RESEARCH_CONFIG_PATH="$REPO_ROOT/config.yaml"
fi

if [ ! -f "$YANDEX_DEEP_RESEARCH_CONFIG_PATH" ]; then
    # Try to seed from repo (config.example.yaml is the canonical template)
    if [ -f "$REPO_ROOT/config.example.yaml" ]; then
        cp "$REPO_ROOT/config.example.yaml" "$YANDEX_DEEP_RESEARCH_CONFIG_PATH"
        echo -e "${GREEN}✓ Seeded config.example.yaml → $YANDEX_DEEP_RESEARCH_CONFIG_PATH${NC}"
        echo -e "${YELLOW}⚠ config.yaml was seeded from the example template.${NC}"
        echo "  Edit $YANDEX_DEEP_RESEARCH_CONFIG_PATH and set your model API keys before use."
    else
    ...
    echo -e "${GREEN}✓ config.yaml: $YANDEX_DEEP_RESEARCH_CONFIG_PATH${NC}"
fi

# ── extensions_config.json ───────────────────────────────────────────────────

if [ -z "$YANDEX_DEEP_RESEARCH_EXTENSIONS_CONFIG_PATH" ]; then
    export YANDEX_DEEP_RESEARCH_EXTENSIONS_CONFIG_PATH="$REPO_ROOT/extensions_config.json"
fi

if [ ! -f "$YANDEX_DEEP_RESEARCH_EXTENSIONS_CONFIG_PATH" ]; then
    if [ -f "$REPO_ROOT/extensions_config.json" ]; then
        cp "$REPO_ROOT/extensions_config.json" "$YANDEX_DEEP_RESEARCH_EXTENSIONS_CONFIG_PATH"
        echo -e "${GREEN}✓ Seeded extensions_config.json → $YANDEX_DEEP_RESEARCH_EXTENSIONS_CONFIG_PATH${NC}"
    else
        # Create a minimal empty config so the gateway doesn't fail on startup
        echo '{"mcpServers":{},"skills":{}}' > "$YANDEX_DEEP_RESEARCH_EXTENSIONS_CONFIG_PATH"
        echo -e "${YELLOW}⚠ extensions_config.json not found, created empty config at $YANDEX_DEEP_RESEARCH_EXTENSIONS_CONFIG_PATH${NC}"
    fi
else
    echo -e "${GREEN}✓ extensions_config.json: $YANDEX_DEEP_RESEARCH_EXTENSIONS_CONFIG_PATH${NC}"
fi
...
_secret_file="$YANDEX_DEEP_RESEARCH_HOME/.better-auth-secret"
...
detect_sandbox_mode() {
    ...
    [ -f "$YANDEX_DEEP_RESEARCH_CONFIG_PATH" ] || { echo "local"; return; }
...
if [ "$CMD" = "down" ]; then
    ...
    export YANDEX_DEEP_RESEARCH_HOME="${YANDEX_DEEP_RESEARCH_HOME:-$REPO_ROOT/backend/.yandex-deep-research}"
    export YANDEX_DEEP_RESEARCH_CONFIG_PATH="${YANDEX_DEEP_RESEARCH_CONFIG_PATH:-$YANDEX_DEEP_RESEARCH_HOME/config.yaml}"
    export YANDEX_DEEP_RESEARCH_EXTENSIONS_CONFIG_PATH="${YANDEX_DEEP_RESEARCH_EXTENSIONS_CONFIG_PATH:-$YANDEX_DEEP_RESEARCH_HOME/extensions_config.json}"
    export YANDEX_DEEP_RESEARCH_DOCKER_SOCKET="${YANDEX_DEEP_RESEARCH_DOCKER_SOCKET:-/var/run/docker.sock}"
    export YANDEX_DEEP_RESEARCH_REPO_ROOT="${YANDEX_DEEP_RESEARCH_REPO_ROOT:-$REPO_ROOT}"
...
if [ "$CMD" = "build" ]; then
    ...
    if [ -z "$YANDEX_DEEP_RESEARCH_DOCKER_SOCKET" ]; then
        export YANDEX_DEEP_RESEARCH_DOCKER_SOCKET="/var/run/docker.sock"
    fi
...
# ── YANDEX_DEEP_RESEARCH_DOCKER_SOCKET ───────────────────────────────────────────────────
...
if [ "$sandbox_mode" != "local" ]; then
    if [ ! -S "$YANDEX_DEEP_RESEARCH_DOCKER_SOCKET" ]; then
        echo -e "${RED}⚠ Docker socket not found at $YANDEX_DEEP_RESEARCH_DOCKER_SOCKET${NC}"
        echo "  AioSandboxProvider (DooD) will not work."
        exit 1
    else
        echo -e "${GREEN}✓ Docker socket: $YANDEX_DEEP_RESEARCH_DOCKER_SOCKET${NC}"
    fi
fi
