...
This document lists common issues encountered during Yandex Deep Research smoke testing and how to resolve them.
...
3. Or stop Yandex Deep Research services first:
   ```bash
   make stop
   ```
...
2. Check nginx logs:
   ```bash
   cd docker && docker compose -p yandex-deep-research-dev -f docker-compose-dev.yaml logs nginx
   ```
...
```bash
docker exec -it yandex-deep-research-gateway sh
```

#### Clean Up All Yandex Deep Research-Related Containers and Images
```bash
make docker-stop
cd docker && docker compose -p yandex-deep-research-dev -f docker-compose-dev.yaml down -v
```
...
1. Check the GitHub issues for the project: https://github.com/yandex/yandex-deep-research/issues
