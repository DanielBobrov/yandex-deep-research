...
:func:`yandex_deep_research.agents.checkpointer.async_provider.make_checkpointer`.

Usage (e.g. FastAPI lifespan)::

    from yandex_deep_research.agents.stream_bridge import make_stream_bridge
...
from yandex_deep_research.config.stream_bridge_config import get_stream_bridge_config
...
async def make_stream_bridge(config=None) -> AsyncIterator[StreamBridge]:
    ...
    if config is None or config.type == "memory":
        from yandex_deep_research.runtime.stream_bridge.memory import MemoryStreamBridge
