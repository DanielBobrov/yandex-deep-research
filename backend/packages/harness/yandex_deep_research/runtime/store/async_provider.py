...
Usage (e.g. FastAPI lifespan)::
    ...
    from yandex_deep_research.runtime.store import make_store
...
from yandex_deep_research.runtime.store.provider import POSTGRES_CONN_REQUIRED, POSTGRES_STORE_INSTALL, SQLITE_STORE_INSTALL, ensure_sqlite_parent_dir, resolve_sqlite_conn_str
...
async def _async_store(config) -> AsyncIterator[BaseStore]:
    ...
    The ``config`` argument is a :class:`yandex_deep_research.config.checkpointer_config.CheckpointerConfig`
...
async def make_store() -> AsyncIterator[BaseStore]:
    ...
    :func:`yandex_deep_research.agents.checkpointer.async_provider.make_checkpointer` so
