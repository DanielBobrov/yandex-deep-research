...
def _normalize_vllm_chat_template_kwargs(payload: dict[str, Any]) -> None:
    """Map YandexDeepResearch's legacy ``thinking`` toggle to vLLM/Qwen's ``enable_thinking``.

    YandexDeepResearch originally documented ``extra_body.chat_template_kwargs.thinking``
