...
class ClaudeChatModel(ChatAnthropic):
    ...
    Config example:
        - name: claude-sonnet-4.6
          use: yandex_deep_research.models.claude_provider:ClaudeChatModel
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        from yandex_deep_research.models.credential_loader import (
    ...
    def _apply_oauth_billing(self, payload: dict) -> None:
        ...
        if "user_id" not in payload["metadata"]:
            ...
            device_id = hashlib.sha256(f"yandex_deep_research-{hostname}".encode()).hexdigest()
            session_id = str(uuid.uuid4())
            payload["metadata"]["user_id"] = json.dumps(
                {
                    "device_id": device_id,
                    "account_uuid": "yandex_deep_research",
