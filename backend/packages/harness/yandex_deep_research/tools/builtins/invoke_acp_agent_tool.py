...
def _build_mcp_servers() -> dict[str, dict[str, Any]]:
    """Build ACP ``mcpServers`` config from YandexDeepResearch's enabled MCP servers."""
...
def _build_acp_mcp_servers() -> list[dict[str, Any]]:
    ...
    The ACP client expects a list of server objects, while YandexDeepResearch's MCP helper
