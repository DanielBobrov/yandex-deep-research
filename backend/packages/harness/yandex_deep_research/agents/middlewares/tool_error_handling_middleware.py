...
def _build_runtime_middlewares(
    *,
    ...
    from yandex_deep_research.agents.middlewares.llm_error_handling_middleware import LLMErrorHandlingMiddleware
    ...
    from yandex_deep_research.config.guardrails_config import get_guardrails_config
        ...
        if "framework" not in provider_kwargs:
            try:
                ...
                if "framework" in sig.parameters or any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values()):
                    provider_kwargs["framework"] = "yandex_deep_research"
    ...
    from yandex_deep_research.agents.middlewares.sandbox_audit_middleware import SandboxAuditMiddleware
