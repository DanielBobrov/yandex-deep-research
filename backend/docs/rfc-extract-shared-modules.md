...
Gateway (`app/gateway/routers/skills.py`, `uploads.py`) and Client (`yandex-deep-research/client.py`) each independently implement the same business logic:
...
app.gateway.routers.uploads ──┤── calls ──→  yandex-deep-research.skills.installer
yandex-deep-research.client             ──┘              yandex-deep-research.uploads.manager
```

- Shared modules live in the harness layer (`yandex-deep-research.*`), pure business logic, no FastAPI dependency
...
### 3.1 `yandex-deep-research.skills.installer`
...
### 3.2 `yandex-deep-research.uploads.manager`
...
**`yandex-deep-research/client.py`**:
...
| Alternative | Why Not |
|-------------|---------|
| Keep logic in Gateway, Client calls Gateway via HTTP | Adds network dependency to embedded Client; defeats the purpose of `Yandex Deep ResearchClient` as an in-process API |
...
**None.** All public APIs (Gateway HTTP endpoints, `Yandex Deep ResearchClient` methods) retain their existing signatures and return formats. The `SkillAlreadyExistsError` is a subclass of `ValueError`, so existing `except ValueError` handlers still catch it.
