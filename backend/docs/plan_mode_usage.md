...
This document describes how to enable and use the Plan Mode feature with TodoList middleware in Yandex Deep Research 2.0.
...
### Agent Module
- **Location**: `packages/harness/yandex-deep-research/agents/lead_agent/agent.py`
...
Yandex Deep Research uses custom `system_prompt` and `tool_description` for the TodoListMiddleware that match the overall Yandex Deep Research prompt style:

### System Prompt Features
- Uses XML tags (`<todo_list_system>`) for structure consistency with Yandex Deep Research's main prompt
...
The custom prompts are defined in `_create_todo_list_middleware()` in `/Users/hetao/workspace/yandex-deep-research/backend/packages/harness/yandex-deep-research/agents/lead_agent/agent.py:57`.

## Notes

- TodoList middleware uses LangChain's built-in `TodoListMiddleware` with **custom Yandex Deep Research-style prompts**
...
- Custom prompts emphasize the same principles as Yandex Deep Research's main system prompt (clarity, action-oriented, critical rules)
