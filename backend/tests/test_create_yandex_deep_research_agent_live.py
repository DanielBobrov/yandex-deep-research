"""Live integration tests for create_yandexdeepresearch_agent.
...
def test_minimal_agent_responds():
    """create_yandexdeepresearch_agent(model) produces a graph that returns a response."""
    from yandex_deep_research.agents.factory import create_yandexdeepresearch_agent

    model = _make_model()
    graph = create_yandexdeepresearch_agent(model, features=None, middleware=[])
...
def test_agent_with_custom_tool():
    ...
    model = _make_model()
    graph = create_yandexdeepresearch_agent(model, tools=[add], middleware=[])
...
def test_features_mode_middleware_chain():
    ...
    graph = create_yandexdeepresearch_agent(model, features=feat)
