...
from yandex_deep_research.sandbox.tools import (
    VIRTUAL_PATH_PREFIX,
    _apply_cwd_prefix,
...
_THREAD_DATA = {
    "workspace_path": "/tmp/yandex-deep-research/threads/t1/user-data/workspace",
    "uploads_path": "/tmp/yandex-deep-research/threads/t1/user-data/uploads",
    "outputs_path": "/tmp/yandex-deep-research/threads/t1/user-data/outputs",
}
...
def test_replace_virtual_path_maps_virtual_root_and_subpaths() -> None:
    assert Path(replace_virtual_path("/mnt/user-data/workspace/a.txt", _THREAD_DATA)).as_posix() == "/tmp/yandex-deep-research/threads/t1/user-data/workspace/a.txt"
    assert Path(replace_virtual_path("/mnt/user-data", _THREAD_DATA)).as_posix() == "/tmp/yandex-deep-research/threads/t1/user-data"
...
def test_replace_virtual_path_preserves_trailing_slash() -> None:
    ...
    assert result == "/tmp/yandex-deep-research/threads/t1/user-data/workspace/"
...
def test_replace_virtual_path_preserves_trailing_slash_windows_style() -> None:
    ...
    """
    win_thread_data = {
        "workspace_path": r"C:\yandex-deep-research\threads\t1\user-data\workspace",
        "uploads_path": r"C:\yandex-deep-research\threads\t1\user-data\uploads",
        "outputs_path": r"C:\yandex-deep-research\threads\t1\user-data\outputs",
    }
...
def test_replace_virtual_path_preserves_windows_style_for_nested_subdir_trailing_slash() -> None:
    ...
    assert result == "C:\\yandex-deep-research\\threads\\t1\\user-data\\workspace\\subdir\\"
    assert "/" not in result, f"Mixed separators in Windows path: {result!r}"
...
def test_replace_virtual_paths_in_command_preserves_trailing_slash() -> None:
    ...
    assert "/tmp/yandex-deep-research/threads/t1/user-data/workspace/" in result, f"Trailing slash lost in: {result!r}"
...
def test_mask_local_paths_in_output_hides_host_paths() -> None:
    output = "Created: /tmp/yandex-deep-research/threads/t1/user-data/workspace/result.txt"
    masked = mask_local_paths_in_output(output, _THREAD_DATA)

    assert "/tmp/yandex-deep-research/threads/t1/user-data" not in masked
...
def test_mask_local_paths_in_output_hides_skills_host_paths() -> None:
    ...
    with (
        ...
        assert "/home/user/yandex-deep-research/skills" not in masked
...
def test_validate_local_tool_path_prioritizes_user_data_before_custom_mounts() -> None:
    from yandex_deep_research.config.sandbox_config import VolumeMountConfig
...
def test_validate_local_tool_path_rejects_none_thread_data() -> None:
    ...
    from yandex_deep_research.sandbox.exceptions import SandboxRuntimeError
...
def test_replace_virtual_paths_in_command_replaces_skills_paths() -> None:
    ...
    with (
        ...
        assert "/home/user/yandex-deep-research/skills/public/bootstrap/SKILL.md" in result
...
def test_replace_virtual_paths_in_command_replaces_both() -> None:
    ...
    with (
        ...
        assert "/tmp/yandex-deep-research/threads/t1/user-data/workspace/out.txt" in result
...
def test_replace_virtual_paths_in_command_replaces_acp_workspace() -> None:
    ...
    with patch("yandex_deep_research.sandbox.tools._get_acp_workspace_host_path", return_value=acp_host):
        ...
        assert "/tmp/yandex-deep-research/threads/t1/user-data/outputs/hello.py" in result
...
def test_apply_cwd_prefix_prepends_workspace() -> None:
    ...
    assert "/tmp/yandex-deep-research/threads/t1/user-data/workspace" in result
...
def test_validate_local_bash_command_paths_allows_mcp_filesystem_paths() -> None:
    ...
    from yandex_deep_research.config.extensions_config import ExtensionsConfig, McpServerConfig
